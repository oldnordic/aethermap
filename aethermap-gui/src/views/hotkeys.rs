...
fn modifier_checkbox<'a>(label: &str, modifier: &str, selected: &'a [String]) -> Element<'a, Message> {
    ...
    let bindings_list = if view.bindings.is_empty() {
        ...
        for (idx, binding) in view.bindings.iter().enumerate() {
            ...
            let rule_row = row![
                indicator,
                column![
                    text(format!("{}+{} \u{2192} {}", modifiers_str, binding.key, binding.profile_name)).size(14),
                    text(format!("Layer: {}",

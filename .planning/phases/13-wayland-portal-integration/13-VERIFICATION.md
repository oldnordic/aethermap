...
| ------------------------------------------------------ | ------------------------------------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `razermapper/razermapper-gui/Cargo.toml`               | ashpd dependency with wayland feature       | ✓ VERIFIED  | ashpd = { version = "0.8", default-features = false, features = ["wayland", "tokio"] } (line 36)                                                          |
| `razermapper/razermapper-gui/src/focus_tracker.rs`     | FocusTracker with portal integration        | ✓ VERIFIED  | 307 lines, FocusEvent with app_id matching, FocusTracker with async task spawning. Graceful fallback when WAYLAND_DISPLAY not set.                        |
| `razermapper/razermapper-common/src/lib.rs`            | Request::FocusChanged and FocusChangedAck   | ✓ VERIFIED  | Request::FocusChanged variant at line 1311, Response::FocusChangedAck with app_id field. IPC serialization tests passing.                                  |
| `razermapper/razermapper-gui/src/ipc.rs`               | send_focus_change() method                  | ✓ VERIFIED  | pub async fn send_focus_change() at line 906, sends Request::FocusChanged via IPC.                                                                         |
| `razermapper/razermapperd/src/auto_profile_switcher.rs`| AutoProfileSwitcher with app_id matching    | ✓ VERIFIED  | 588 lines, apply_focus_change() method with wildcard/prefix/suffix matching. 7 unit tests all passing.                                                     |
| `razermapper/razermapperd/src/config.rs`               | AutoSwitchRule struct for YAML             | ✓ VERIFIED  | AutoSwitchRule with app_id, profile_name, device_id, layer_id fields at line 163. Serialize/Deserialize enabled.                                          |
| `razermapper/razermapperd/src/ipc.rs`                  | FocusChanged request handler                | ✓ VERIFIED  | Handler calls auto_profile_switcher.apply_focus_change() at line 1588.                                                                                    |
| `razermapper/razermapperd/src/global_hotkey_manager.rs`| GlobalHotkeyManager with modifier tracking  | ✓ VERIFIED  | 501 lines, check_key_event() method, modifier tracking with HashSet. 11 unit tests passing.                                                                |
| `razermapper/razermapperd/src/config.rs`               | HotkeyBinding struct for YAML              | ✓ VERIFIED  | HotkeyBinding with modifiers, key, profile_name at line 39. default_hotkey_bindings() provides Ctrl+Alt+Shift+1-9 defaults.                                |
| `razermapper/razermapperd/src/device.rs`               | Hotkey check before remap processing        | ✓ VERIFIED  | Lines 607-622: hotkey_manager.check_key_event() called before remap_engine.process_event(). Matched events consumed via continue.                          |
| `razermapper/razermapper-gui/src/gui.rs`               | AutoSwitchRulesView GUI component           | ✓ VERIFIED  | AutoSwitchRulesView struct at line 126, view methods, message handlers. Navigation buttons wired to ShowAutoSwitchRules (lines 2265, 490).                |
| `razermapper/razermapper-gui/src/gui.rs`               | HotkeyBindingsView GUI component            | ✓ VERIFIED  | HotkeyBindingsView struct at line 162, view methods with modifier checkboxes. Navigation buttons wired to ShowHotkeyBindings (lines 2272, 514).            |
| `razermapper/razermapperd/src/config.rs`               | YAML config examples                        | ✓ VERIFIED  | EXAMPLE_CONFIG_WITH_AUTO_SWITCH constant at line 248 with comprehensive YAML documentation for auto_switch_rules and hotkey_bindings.                      |
...
**Test:** Run razermapper-gui on a Wayland session with xdg-desktop-portal installed

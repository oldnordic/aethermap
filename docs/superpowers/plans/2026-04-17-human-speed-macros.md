...
**Architecture:** Update `aethermap-common` for settings data, `aethermapd` for execution and recording logic, and `aethermap-gui` for user controls.
...
**Files:**
- Modify: `aethermap/aethermap-common/src/lib.rs`
- Modify: `aethermap/aethermap-common/src/ipc_client.rs`

- [ ] **Step 1: Add MacroSettings and update MacroEntry**

Update `aethermap-common/src/lib.rs` to include `MacroSettings` and add `humanize` and `capture_mouse` fields to `MacroEntry`. Add `SetMacroSettings` and `GetMacroSettings` to `Request` and `Response`.

- [ ] **Step 2: Run `cargo check` to verify types**

Run: `cargo check` in `aethermap/aethermap-common`
Expected: PASS (with warnings for unused fields/variants)

- [ ] **Step 3: Update IPC Client**

Update `aethermap/aethermap-common/src/ipc_client.rs` to add async methods for sending and receiving these new macro settings.

- [ ] **Step 4: Commit**

```bash
git add aethermap/aethermap-common/src/lib.rs aethermap/aethermap-common/src/ipc_client.rs
...
**Files:**
- Modify: `aethermap/aethermapd/src/macro_engine.rs`

- [ ] **Step 1: Update MacroEngine struct and execution logic**

Update `aethermap/aethermapd/src/macro_engine.rs` to store `MacroSettings`. Modify the `execute_macro` loop to apply the jitter and latency formula to all `Action::Delay` steps.

- [ ] **Step 2: Implement randomized delay unit test**

Add a test case to `aethermap/aethermapd/src/macro_engine.rs` that verifies delays with jitter fall within the expected range.

- [ ] **Step 3: Run tests**

Run: `cargo test` in `aethermap/aethermapd`
Expected: PASS

- [ ] **Step 4: Commit**

```bash
git add aethermap/aethermapd/src/macro_engine.rs
...
**Files:**
- Modify: `aethermap/aethermapd/src/macro_engine.rs`
- Modify: `aethermap/aethermapd/src/device.rs`

- [ ] **Step 1: Update `process_input_event` to record mouse buttons**

Update `aethermap/aethermapd/src/macro_engine.rs` to handle mouse button codes during recording if `capture_mouse` is enabled.

- [ ] **Step 2: Ensure `device.rs` passes mouse events to recorder**

Modify `aethermap/aethermapd/src/device.rs` to ensure relative axis events and mouse buttons are forwarded to the `MacroEngine` for recording.
...
```bash
git add aethermap/aethermapd/src/macro_engine.rs aethermap/aethermapd/src/device.rs
...
Update `aethermap/aethermap-gui/src/gui.rs` to add sliders for Latency and Jitter, and a checkbox for "Capture Mouse" in the recording section.
...
Run: `cargo check` in `aethermap/aethermap-gui`
Expected: PASS
